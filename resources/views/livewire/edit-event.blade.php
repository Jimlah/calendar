<x-pop-over ::listeners="['edit-event']" @onload="$wire.edit(props.id)" @onclose="$wire.submit(); $wire.close();">
    {{-- Be like water. --}}
    @if($event)
        <form class="text-xs flex flex-col items-start justify-start divide-y" wire:submit.prevent="submit" x-id="['event-input']">
            <label class="px-4 py-1 w-full inline-block" :for="$id('text-input')">
                <span class="sr-only">Event Name</span>
                <textarea wire:model.lazy="event.name" :id="$id('text-input')" x-autosize type="text" class="border-none h-fit text-lg inline-block focus:outline-none focus:ring-none caret-black resize-none" ></textarea>
            </label>
                ...
                <div x-data="{isOpen: false}" @click.outside="isOpen=false">
                    ...
                    <div x-show="isOpen" class="flex items-start justify-start flex-col space-y-2">
                        <label class="flex items-center justify-start w-full" >
                            ...
                            <span class="w-full">
                                <input name="is_all_day" type="checkbox" class="focus:outline-none ring-0" wire:model.lazy="event.is_all_day" />
                            </span>
                        </label>
                        <label class="flex items-center justify-start" >
                            <span class="text-xs inline-block text-right mr-2 w-1/3 flex-none">Starts:</span>
                            <input name="start_at" type="datetime-local" class="focus:outline-none ring-0 w-full" wire:model.lazy="event.start_at"/>
                        </label>
                        <label class="flex items-center justify-start" >
                            <span class="text-xs inline-block text-right mr-2 w-1/3 flex-none">Ends:</span>
                            <input name="ends_at" type="datetime-local" class="focus:outline-none ring-0 w-full" wire:model.lazy="event.end_at" />
                        </label>
                    </div>
                </div>
            </div>
